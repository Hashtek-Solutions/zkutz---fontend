...
const Navbar = () => {
  ...
  return (
    <nav className="fixed left-1/2 top-4 z-10 w-[95%] max-w-screen-xs -translate-x-1/2 overflow-hidden rounded-xl border border-gray-700 bg-white/10 bg-opacity-90 px-4 py-2 shadow backdrop-blur-sm backdrop-saturate-150 md:max-w-screen-lg lg:max-w-screen-xl lg:px-8 lg:py-3">
      <div className="container mx-auto flex flex-wrap items-center justify-between">
        <Link href="/" className="mr-4 block cursor-pointer py-1.5 text-base font-semibold">
          <Image src="/logo.svg" alt="Logo" width={90} height={50} />
        </Link>
        <div className="hidden lg:block">
          <ul className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
            {navItems.map((item, index) => (
              <li key={index} className="flex items-center gap-x-2 p-1 text-sm text-white">
